...
//main class
public class ReceptionistCheckOut extends JFrame implements ItemListener, ActionListener {
	// global variable
	...
	JButton btnView, btnCheckout, btnClose;
	JComboBox cmbRoomNo;

	DefaultTableModel dtm = new DefaultTableModel();
	JTable table = new JTable(dtm);
	ArrayList<NCbookLibs> arrayCheckOut;
	...
	public ReceptionistCheckOut() {
		...
		lblTitle = new JLabel("CHECKOUT CUSTOMERS ");
		lblTitle.setBounds(445, 0, 300, 40);
		...
		btnCheckout.setFont(new Font("Arial", Font.PLAIN, 12));
		btnCheckout.setBounds(730, 10, 100, 23);
	...
	public void refreshTable() {
		arrayCheckOut = new NCbookJDBC().getCheckedIn();
		dtm.setRowCount(0);
		for (int i = 0; i < arrayCheckOut.size(); i++) {
			NCbookLibs checkin = arrayCheckOut.get(i);
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource() == btnCheckout) {
			...
			booklibs.setBookingID(Integer.parseInt(txtID.getText()));
			NCbookJDBC jdbc = new NCbookJDBC();
			boolean result = jdbc.checkOut(booklibs);
			if (result == true) {
				JOptionPane.showMessageDialog(null, "Checked out successful");
				txtID.setText(null);
				...
				JOptionPane.showMessageDialog(null, "Checked out unsuccessful");
			}
		} else if (ae.getSource() == btnView) {
			new ReceptionistViewCheckOut();
